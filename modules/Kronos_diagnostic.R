...
option_list = list(
  ...
  make_option(
    ...
    action = "store_true",
    default = F,
    help = "If True, diagnostic will run in developer mode to ignore confidence ploidy and export metadata. Created especially for MnM. [default= %default]",
    metavar = "logical"
  )
)
...
if(opt$developer_mode == T){
  if ('threshold_Sphase' %in% names(opt) | 'threshold_G1G2phase' %in% names(opt) ){
    data=data %>%
      mutate(Type = ifelse(
    ...
    median_G1 = median(data%>%filter(Type != 'S-phase cells' & normalized_dimapd < opt$threshold_G1G2phase)%>%pull(mean_ploidy))
    print(median_G1)
    data = data %>%
      mutate(
    ...
    data=data%>%
      mutate(Type= case_when(
        ...
        # ploidy_confidence < 2 & ploidy_confidence!=-100 & !is.na(ploidy_confidence) ~ 'Low ploidy confidence',
        # mean_ploidy < median_ploidy_not_noisy / 1.5 ~ 'Low ploidy compared to G1/G2-phase pool',
        # mean_ploidy > median_ploidy_not_noisy * 2 ~ 'High ploidy compared to G1/G2-phase pool',
        T ~ Type
      ))
    ...
    p=data%>%
      ...
      scale_color_manual(
        values = c(
          ...
          # 'Low ploidy compared to G1/G2-phase pool' = "#01e7ab",
          # 'High ploidy compared to G1/G2-phase pool' ="#a7001b",
    ...
    suppressMessages( ggsave(p, filename = paste0(file.path(opt$out, opt$base_name), '_no_correction_plot_dev.pdf')))
    write_tsv(data ,paste0(file.path(opt$out,opt$base_name),'_phases_from_diagnostic.tsv'))
    print('done')
    quit()
  }else{
    ...
    # median_ploidy_not_noisy = median(data%>%filter(is_noisy == F)%>%pull(mean_ploidy))
    median_ploidy_not_noisy = median(data%>%filter( (is_noisy == F | is.na(is_noisy)))%>%pull(mean_ploidy))
    print(median_ploidy_not_noisy)
    
    data=data%>%
      mutate(Type= case_when(
        (is.na(is_noisy) & coverage_per_1Mbp >= opt$min_n_reads*median_ploidy_not_noisy) ~ 'unknown cells',
        (is.na(ploidy_confidence) & coverage_per_1Mbp >= opt$min_n_reads*median_ploidy_not_noisy) ~ 'unknown cells',
        coverage_per_1Mbp < opt$min_n_reads*median_ploidy_not_noisy ~ 'Low Coverage',
        # ploidy_confidence < 2 & ploidy_confidence!=-100 & !is.na(ploidy_confidence) ~ 'Low ploidy confidence',
        # mean_ploidy < median_ploidy_not_noisy / 1.5 ~ 'Low ploidy compared to G1/G2-phase pool',
        # mean_ploidy > median_ploidy_not_noisy * 2 ~ 'High ploidy compared to G1/G2-phase pool',
...
if (!'threshold_Sphase' %in% names(opt)){
  ...
  data=data%>%
    filter(coverage_per_1Mbp >= opt$min_n_reads*median_ploidy_not_noisy#,
           # ploidy_confidence > 2 | ploidy_confidence==-100 | is.na(ploidy_confidence),
