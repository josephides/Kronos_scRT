...
if (grepl(x = extract_unit, pattern =  '[0-9][0-9]')) {
  ...
  opt$binSize = case_when(
    ...
    n_of_zeros >= 6 ~ paste0(str_remove(opt$binSize, '0{6}$'), 'Mb')
  )
} else{
...
suppressMessages(ggsave(
  p,
  filename = paste0(file.path(opt$out, opt$output_file_base_name), '_plot.png')
))
...
suppressMessages(ggsave(
  ...
  filename = paste0(file.path(
    opt$out,
    opt$output_file_base_name),
    '_percentage_of_replicating_cells_used_for_RT_calculation.png'
  )
))
...
suppressMessages(ggsave(
  ...
  filename = paste0(file.path( opt$out,
                               opt$output_file_base_name),
                    '_Twidths_extended.png'),width = 2.5*ncat,height = 4*nbasen
))
...
if (opt$Var_against_reference) {
  ...
  suppressMessages(ggsave(
    ...
    filename = paste0(file.path( opt$out,
                                 opt$output_file_base_name),
                      '_Twidths_extended_ref_RT.png'),width = 2.2*ncat,height = 4*nbasen
  ))
